...
- `algorithmic-problem-solving/` is the static HTML learning area:
  ...
  - `problem-journal/index.html` uses `problem-journal/Sem_X/Contest_N/Task_K/index.html`; the `Sem_X/` and `Contest_N/` directories also have indexes.
...
- A task page should cross-link the relevant pattern pages, existing theory under `/conspecta/`, external solution code, applicable mistake entries, and review items. Use `<time datetime="YYYY-MM-DD">` for machine-readable dates.
