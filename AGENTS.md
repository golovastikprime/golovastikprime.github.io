...
- `index.html` is the theory-library homepage. `styles.css` is the shared visual system; `app.js` powers only the homepage search and semester filters. `math.js` configures MathJax for published task notes.
...
- `.github/workflows/compile_typst_to_pdf.yml` compiles Typst entrypoints and then publishes the repository root to GitHub Pages. There is no general bundling step or package-manager dependency. Published task notes load the pinned MathJax 4.1.2 browser bundle from jsDelivr.
...
- Keep pages as UTF-8 static HTML. Reuse `styles.css`, the established typography, and the existing header/footer structure; do not introduce a framework or another production dependency without approval.
...
- Use TeX inside `.math-inline` with `\(...\)` and inside `.math-display` with `\[...\]`; published task pages load `/math.js` before the pinned MathJax browser bundle. Use proper TeX environments such as `aligned` and `cases` instead of hand-built Unicode layouts. Reserve `<code>` for source code, pseudocode, filenames, and repository identifiers.
...
- Run the site checker, all four JavaScript syntax checks, and `git diff --check` after relevant edits.
